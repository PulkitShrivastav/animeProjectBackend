...
function myHTMLmssg(otp: string, name: string) {
    const mssg = `
    <!DOCTYPE html>
    <html>
    <head>
...
login_route.put('/send_otp', async (req: express.Request, res: express.Response) => {
    ...
    const result = await db.query<Record<'guest_id' | 'otp_code', any>>(query, [email_address, verify_code, 1])
...
login_route.put('/login', async (req: express.Request, res: express.Response) => {
    ...
    const newToken = getToken({ user_id: userID, user_email: email_address })
    ...
    const ref_token = refToken({ user_id: userID, user_email: email_address })
...
login_route.put('/resend_otp', async (req: express.Request, res: express.Response) => {
